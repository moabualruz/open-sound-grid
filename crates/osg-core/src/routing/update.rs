...
impl MixerSession {
    ...
    pub fn update(
        ...
        let output = match message {
            StateMsg::AddEphemeralNode(id, kind) => self.handle_add_ephemeral_node(id, kind, graph),
            StateMsg::AddChannel(name, kind) => {
                self.handle_add_channel(name, kind, &mut pw_messages)
            }
            StateMsg::AddApp(id, kind) => self.handle_add_app(id, kind, graph, settings),
            StateMsg::RemoveEndpoint(ep) => {
                self.handle_remove_endpoint(ep, graph, settings, &mut pw_messages)
            }
            StateMsg::SetVolume(ep_desc, volume) => {
                self.handle_set_volume(ep_desc, volume, graph, settings, &mut pw_messages);
                None
            }
            ...
            StateMsg::SetMute(ep_desc, muted) => {
                self.handle_set_mute(ep_desc, muted, graph, settings, &mut pw_messages);
                None
            }
            StateMsg::SetVolumeLocked(ep_desc, locked) => {
                self.handle_set_volume_locked(ep_desc, locked, graph, settings, &mut pw_messages);
                None
            }
            StateMsg::Link(source, sink) => {
                self.handle_link(source, sink, &mut pw_messages);
                None
            }
            ...
            StateMsg::SetLinkLocked(source, sink, locked) => {
                self.handle_set_link_locked(source, sink, locked);
                None
            }
            StateMsg::ChangeChannelKind(id, kind) => {
                self.handle_change_channel_kind(id, kind, &mut pw_messages);
                None
            }
            StateMsg::RenameEndpoint(descriptor, name) => {
                self.handle_rename_endpoint(descriptor, name, &mut pw_messages);
                None
            }
            ...
            StateMsg::SetMixOutput(channel_id, output_node_id) => {
                pw_messages.extend(self.handle_set_mix_output(channel_id, output_node_id, graph));
                None
            }
            ...
            StateMsg::SetDefaultOutputNode(node_id) => {
                self.handle_set_default_output_node(node_id);
                None
            }
