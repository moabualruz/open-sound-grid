...
use super::{GroupNodeKind, identifier::NodeIdentifier, pod::DeviceActiveRoute};
...
impl ObjectConvertErrorExt for GlobalObject<&DictRef> {
    fn check_type(
        &self,
        expected: ObjectType,
        expected_str: &'static str,
...
impl<P: Debug, L> Debug for Device<P, L> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("Device")
            ...
            .field("active_routes", &self.active_routes)
            .field("proxy", &self.proxy)
            .finish_non_exhaustive()
    }
}
...
impl<P: Debug, L> Debug for Node<P, L> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("Node")
            .field("id", &self.id)
            ...
            .field("channel_volumes", &self.channel_volumes)
