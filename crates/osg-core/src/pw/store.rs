...
pub(super) struct Store {
    ...
    /// This map is used to store proxies of group nodes created by OSG. They will be
...
impl Store {
    ...
    pub(super) fn set_osg_client_id(&mut self, id: u32) {
    ...
    pub(super) fn remove_object(&mut self, id: u32) {
        if let Some(client) = self.clients.remove(&id) {
            // Check if the client being removed is OSG. If so, remove its id.
            if client.is_osg {
                self.osg_client_id = None;
            }
    ...
    pub(super) fn add_client(
        ...
        // Check if the client is OSG. If so, mark it as such.
        if Some(client.id) == self.osg_client_id {
            client.is_osg = true;
        }
